...
```bash
# Example 1: Minimal agent (URL extraction) — Listing 2.7
python scripts/example_01_minimal_agent.py

# Safety gate demo (three-outcome: blocked / approved / denied)
python scripts/example_safety_gate.py

# Kill switch demo — check .is_active in an agent loop
python scripts/example_kill_switch.py

# Example 5: Artifact logging — Listing 2.9
python scripts/example_05_artifact_logging.py

# Example 8: NMAP triage workflow (Section 2.6 support code)
python scripts/example_08_nmap_triage.py

# Chapter exercise starter (Exercise 2.1)
...
│   ├── adapters/           # Framework adapters (populated in later chapters)
...
│   ├── example_01b_minimal_agent_verbose.py # Annotated walkthrough
│   ├── example_05_artifact_logging.py       # Listing 2.9
│   ├── example_safety_gate.py               # Safety gate (Listing 2.8)
│   ├── example_kill_switch.py               # Kill switch (Listing 2.10)
│   ├── example_08_nmap_triage.py            # Section 2.6 triage workflow
│   └── exercise_2_1_starter.py             # Chapter exercise starter
...
│   └── targets.py                   # Sample hostnames/IPs for examples
├── assets/                 # Supporting assets
│   └── targets.json                 # JSON target list for examples
├── tests/                  # Unit tests
...
See [`docs/listings_reference.md`](docs/listings_reference.md) for complete details and [`docs/exercise_2_2_checklist.md`](docs/exercise_2_2_checklist.md) for the Exercise 2.2 checklist.
...
Shows the three outcomes of `safety_gate` from Listing 2.8:
- Blocked: exact prohibited hostname → auto-denied, no prompt
- Approved: operator types `y` at the confirmation prompt
- Denied: operator types `n` at the confirmation prompt
...
Demonstrates the Listing 2.10 pattern:
- Starts the kill switch monitor in a daemon thread
- Agent loop checks `kill_switch.is_active` each iteration
- Type `STOP` and press Enter to trigger a clean shutdown
- **No network calls, no API keys required**
...
- ✅ The prohibited hostname set in `safety_gate` is defined in `src/safety/gates.py` — update it before deploying
